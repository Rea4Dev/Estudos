...
def percorrer_bd(historico, dados_bd, lista_historico):
    ...
    print(f"\033[0;37m{"_"*TABULACAO_MENOR} Percorrendo Banco de Dados: \033[1;34mlinha do Banco de Dados\033[0;37m{"_"*TABULACAO_MENOR}")

    #action_id_bd = int(linha_bd[13])
    #result_linha_bd = str(linha_bd[8])

    percorrer_hist(historico, lista_historico, dados_bd)

def percorrer_hist(historico, lista_historico, dados_bd):
    for n, linha in enumerate(lista_historico[1:]):
        ...
        if (lista_historico.count(action_id_linha_hist) > 1):
            #time.sleep(0.1)   
            ...
            print(f"IDs diferentes. Linha ignorada.\n") 
